...
class AssetAnalysisViewModel extends ChangeNotifier {
  ...
  List<MonthlyForecast> getFutureBalancesForecast() {
    ...
    if (_incomes.isNotEmpty) {
      ...
      avgMonthlyIncome = monthlyIncomes.values.fold(0.0, (double sum, amount) => sum + amount) / monthlyIncomes.length;
    }
    ...
    if (_expenses.isNotEmpty) {
      ...
      avgMonthlyExpense = monthlyExpenses.values.fold(0.0, (double sum, amount) => sum + amount) / monthlyExpenses.length;
    }
      // NISA投資の月々の拠出総額
    double monthlyNisaContribution = _investments.fold(
      0.0, (double sum, investment) => sum + investment.monthlyContribution);
    ...
    double cumulativeNisa = _investments.fold(
      0, (sum, investment) => sum + investment.currentValue); // 現在のNISA評価額
    ...
    for (int i = 0; i < _forecastMonths; i++) {
      final forecastMonth = DateTime(currentMonth.year, currentMonth.month + i, 1);
      ...
      cumulativeNisa = cumulativeNisa * (1 + monthlyNisaGrowthRate) + monthlyNisaContribution;
  ...
  // NISAの将来価値予測を計算
  List<NisaForecast> getNisaForecast() {    // 現在のNISA投資合計
    double totalCurrentValue = _investments.fold(
      0.0, (double sum, investment) => sum + investment.currentValue);
    ...
    for (int i = 0; i < _forecastMonths; i++) {
      ...
      cumulativeValue = cumulativeValue * (1 + monthlyGrowthRate) + monthlyContribution;
      
      forecasts.add(NisaForecast(
        month: forecastMonth,
