## 🎯 Wikiへようこそ

**MoneyG Finance App Wiki**は、プロジェクトの詳細な技術情報、開発ガイドライン、そして最新の開発状況を提供する包括的なドキュメントハブです。

### 📚 このWikiの目的

- 🛣️ **開発ロードマップ**: プロジェクトの方向性と計画の明確化
- 📊 **現在の状況**: 開発進捗とマイルストーンの可視化
- 📖 **技術ドキュメント**: アーキテクチャ、API、実装詳細の文書化
- 🤝 **コラボレーション**: チーム開発とコミュニティ貢献の促進

---

## 🚀 MoneyG Finance App とは

### 💡 プロジェクト概要

MoneyG Finance Appは、**プライバシーを最優先**に設計された個人家計管理アプリケーションです。Flutter技術を活用してAndroid・iOS両プラットフォームに対応し、すべてのデータをローカルに保存することでユーザーのプライバシーを完全に保護します。

### 🌟 主要特徴

- 🔒 **完全ローカル保存**: クラウド同期なし、プライバシー保護
- 📱 **クロスプラットフォーム**: Android・iOS両対応
- 💰 **包括的な家計管理**: 収支管理、予算設定、資産分析
- 📊 **視覚的な分析**: チャート・グラフによる直感的な把握
- 🎨 **モダンUI**: Material Design準拠の美しいインターフェース
- 🌐 **多言語対応**: 日本語メイン、英語対応予定

### 🎯 ターゲットユーザー

- プライバシーを重視する個人・家庭
- 詳細な家計分析を求めるユーザー
- シンプルで直感的なUIを好むユーザー
- クロスプラットフォーム対応を必要とするユーザー

---

## 📋 Wiki コンテンツ構成

### 🏗️ 開発関連

| カテゴリ | 内容 | 対象者 |
|---------|------|--------|
| **[アーキテクチャ](Architecture)** | システム設計、データフロー | 開発者 |
| **[API リファレンス](API-Reference)** | 内部API、メソッド仕様 | 開発者 |
| **[開発環境セットアップ](Development-Setup)** | 環境構築、ツール設定 | 新規開発者 |
| **[コーディング規約](Coding-Standards)** | スタイルガイド、ベストプラクティス | 全開発者 |

### 📖 ユーザー向け

| カテゴリ | 内容 | 対象者 |
|---------|------|--------|
| **[ユーザーガイド](User-Guide)** | 使い方、機能説明 | エンドユーザー |
| **[FAQ](FAQ)** | よくある質問と回答 | 全ユーザー |
| **[トラブルシューティング](Troubleshooting)** | 問題解決方法 | ユーザー・開発者 |
| **[インストールガイド](Installation-Guide)** | アプリのインストール方法 | 新規ユーザー |

### 🛣️ プロジェクト管理

| カテゴリ | 内容 | 対象者 |
|---------|------|--------|
| **[ロードマップ](Roadmap)** | 開発計画、将来展望 | 全関係者 |
| **[リリースノート](Release-Notes)** | バージョン履歴、変更点 | 全ユーザー |
| **[マイルストーン](Milestones)** | 開発目標、進捗状況 | 開発チーム |
| **[貢献ガイド](Contributing)** | コントリビューション方法 | コントリビューター |

---

## 📊 現在の開発状況

### 🎯 最新バージョン: v1.3.0

**リリース日**: 2025年6月22日

#### ✨ 主要機能
- ✅ 収支管理（収入・支出の記録）
- ✅ カテゴリ別分析
- ✅ 月次・年次レポート
- ✅ 予算設定・追跡
- ✅ 資産分析
- ✅ CSV インポート・エクスポート
- ✅ NISA投資記録
- ✅ 🎨 **テーマカスタマイズ** (v1.3.0)
- ✅ 9つのプリセットテーマ (v1.3.0)
- ✅ カスタムテーマ作成機能 (v1.3.0)

#### 🚧 開発中の機能

- 🔄 iOS版リリース
- 🔄 Web版対応
- 🔄 テーマ共有機能
- 🔄 多言語対応拡張

### 📈 技術指標

| 指標 | 現在値 | 目標値 |
|------|--------|--------|
| **テストカバレッジ** | 85% | 90% |
| **ビルド成功率** | 98% | 99% |
| **起動時間** | <2秒 | <1.5秒 |
| **メモリ使用量** | 50MB | 40MB |

---

## 🤝 コミュニティ・貢献

### 👥 コントリビューター募集

MoneyG Finance Appは、オープンソースプロジェクトとしてコミュニティの貢献を歓迎しています！

#### 🌟 貢献方法

- 🐛 **バグ報告**: [Issues](https://github.com/paraccoli/flutter_finance_app/issues)
- 🔧 **コード貢献**: [Pull Requests](https://github.com/paraccoli/flutter_finance_app/pulls)
- 📚 **ドキュメント**: Wiki・README改善

#### 📋 始める前に

- **[コントリビューションガイド](https://github.com/paraccoli/flutter_finance_app/blob/main/CONTRIBUTING.md)** を必読してください。

---

## 📞 サポート・連絡先

### 💬 質問・サポート

- **一般的な質問**: [GitHub Discussions](https://github.com/paraccoli/flutter_finance_app/discussions)
- **バグ報告**: [GitHub Issues](https://github.com/paraccoli/flutter_finance_app/issues)

### 📧 開発者連絡先

- **GitHub**: [@paraccoli](https://github.com/paraccoli)
- **Twitter**: [@paraccoli](https://twitter.com/paraccoli)
- **プロジェクト**: [flutter_finance_app](https://github.com/paraccoli/flutter_finance_app)

### 🔗 関連リンク

- **メインリポジトリ**: [GitHub](https://github.com/paraccoli/flutter_finance_app)
- **プライバシーポリシー**: [PRIVACY.md](https://github.com/paraccoli/flutter_finance_app/blob/main/PRIVACY.md)
- **セキュリティポリシー**: [SECURITY.md](https://github.com/paraccoli/flutter_finance_app/blob/main/SECURITY.md)
- **ライセンス**: [LICENSE](https://github.com/paraccoli/flutter_finance_app/blob/main/LICENSE)

---

## 📚 その他のリソース

### 🛠️ 技術リソース

- **Flutter公式**: [flutter.dev](https://flutter.dev/)
- **Dart言語**: [dart.dev](https://dart.dev/)
- **Material Design**: [material.io](https://material.io/design)

### 📱 プラットフォーム情報

- **Android**: API Level 21+ (Android 5.0+)
- **iOS**: iOS 13.0+
- **Web**: 将来対応予定

### 📊 プロジェクト統計

- **開始日**: 2024年12月
- **言語**: Dart (Flutter)
- **ライセンス**: MIT License
- **プラットフォーム**: Android、iOS

---

**📝 最終更新**: 2025年6月20日  
**📄 Wiki バージョン**: v1.0  
**👥 メンテナー**: [@paraccoli](https://github.com/paraccoli)

> **MoneyG Finance App Wiki**にようこそ！  
> このWikiが、あなたのプロジェクト理解と貢献の助けとなることを願っています。 🚀

---

*📌 **注意**: このWikiは継続的に更新されます。最新情報は定期的にご確認ください。*
